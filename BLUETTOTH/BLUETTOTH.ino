...
SoftwareSerial Bluetooth(10, 11); // RX, TX - Bluetooth için yazılım serisi
...
bool autoMode = false; // Otonom modda mı kontrolü
...
void setup() {
    Serial.begin(9600);
    Bluetooth.begin(9600); // Bluetooth modülü seri iletişimini başlat
    pinMode(Echo1, INPUT);
...
void loop() {
    ...
    if (Bluetooth.available() > 0) {
        ...
        if (command == 'S') { // S komutu ile araç çalışmaya başlar ve otonom moda geçer
            autoMode = true;
        }
    }

    if (!autoMode) {
        _Stop(); // Otonom moda geçmediyse araç durur
        return;
    }
    ...
    // Durma koşulu
    if (Middle_Distance > 30 && Right_Distance > 70 && Left_Distance > 70) {
        _Stop();
    }
    // İleri gitme koşulları
    else if ((Middle_Distance > 20 && Right_Distance > 20 && Left_Distance > 20) || (Middle_Distance > Right_Distance && Middle_Distance > Left_Distance)) {
        _Forward();
    }
    // Sağa dönüş koşulu
    else if (Left_Distance < 20 && Middle_Distance < 20 && Right_Distance > 20) {
        _Back();
        delay(100);
        _right();
        delay(130);
    ...
    // Sola dönüş koşulu
    else if (Right_Distance < 20 && Middle_Distance < 20 && Left_Distance > 20) {
        _Back();
        delay(100);
        _left();
        delay(130);
